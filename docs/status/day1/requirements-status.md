...
| A11 | Install pulita | Su una VPS Linux vuota, l’installazione è un’esperienza pulita: un comando, gateway persistente, aggiornamento e rollback? | **PARZIALE — il percorso host è verificato da GitHub Actions, la VPS reale no.** L’install workflow sul candidato corrente `dev@8c6d8f8` è passato il 25/09 ([run #36092829833](https://github.com/muffin-project/muffin-agent/actions/runs/36092829833)): installa da una home temporanea senza Node, esegue doctor e update/rollback, verifica la unit con `systemd-analyze` e avvia il suo `ExecStart` in foreground. Il runner non ha un bus systemd utente, quindi questo non prova persistenza dopo logout/riavvio né un vero VPS; non prova nemmeno macOS/launchd o il bootstrap root. La fresh-VPS pubblica e le decisioni root/AppArmor sono post-source-public e restano in #654. |
