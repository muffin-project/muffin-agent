...
1. **Il segnale.** L'issue più votata chiede letteralmente *"You went quiet after that thread — want a recap?"* — cioè `gone_quiet`, chiesto da un loro utente, mentre in core **niente osserva la persona** (`silent_since`, absence: not found). E le issue "troppo invadente" sono **zero**.
2. **La forma.** Un utente ha costruito il layer **fuori** dal prodotto (hermes-active): l'LLM viene *chiesto*, una **matrice deterministica decide se esce**, **70% di SKIP** in produzione — con la riga chiave *"non ho trovato un modo pulito di farlo col solo cron"*. È, per struttura, "il modello propone, il codice decide": esattamente lo Stadio-1/Stadio-2 di `core/scheduler/proactivity.ts`, derivato indipendentemente da chi aveva bisogno che funzionasse.
