...
- **Niente side-channel** (lezione Hermes #4146: escape via canale RPC interno verso un tool privilegiato): il codice dentro il sandbox non ha alcun canale verso i tool del loop — è codice+fs+rete proxata. Il confine di esecuzione è anche un confine di capability.
