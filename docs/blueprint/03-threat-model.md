...
> Postura di partenza (consenso OWASP + Willison, A3 §7): **l'indirect prompt injection non si patcha nel prompt — si contiene nell'architettura**. Assumiamo che l'iniezione *dentro il modello* prima o poi riesca; il design garantisce che un modello compromesso in-context possa comunque fare solo ciò che il kernel di policy consente a quel contesto. "Kernel = ciò che il modello non può bypassare" (direzione interna già formulata, qui resa struttura). Decisioni: ADR-0003 (Root of Trust), ADR-0009 (kernel di policy unificato), ADR-0005 (taint), ADR-0044 su `slice/taint-in-ingresso` non ancora integrata (taint obbligatorio all'ingresso dei tool), ADR-0046 (identità e contenuto sono due piani distinti).
...
| Input non fidato | **ogni campo letto**: messaggi, nomi, bio, username, titoli di stanze, file e filename, web/tool/MCP, metadati, immagini, OCR, audio e trascrizioni |
...
- **Principal del turno**: `owner@host` (CLI o canale privato autenticato), `member@group:<id>`, `system@scheduler` (trigger interni), `agent@dev` (lavoro su se stesso). Il principal + il tenant determinano lo **snapshot di permessi** del turno, calcolato nel pre-loop e immutabile per il turno (V7). Su una surface remota `owner` deriva solo da un binding protetto `(connector, issuer/installazione, subject-id stabile autenticato)` creato dal control plane locale. Display name, bio, username, chat id, nome stanza, foto e contenuto non partecipano mai all'autorità. Una surface senza un identificatore stabile autenticato non può produrre un principal owner. Il re-binding è una cerimonia esplicita e auditata dal control plane, mai una richiesta conversazionale.
- **Piano identità ≠ piano contenuto** (ADR-0046). Il connector risolve il principal dall'envelope autenticato e, separatamente, parsa **ogni campo che renderà visibile al modello** in blocchi tipizzati con fonte e tier. Testo, caption, quote, forward, nomi, bio, filename, MIME/EXIF, alt text, OCR, descrizioni di immagini, trascrizioni e metadata sono contenuto. Se un formato non è supportato, il dato viene rifiutato o messo in quarantena con un errore visibile; i byte raw non vengono promossi a istruzioni. **Parsato non significa sanificato**: l'iniezione resta un'ipotesi di progetto e viene contenuta da provenance, spotlighting, taint e kernel.
- **Trust tier** (dal modello dati, 02 §5): 0 owner · 1 contatti confermati · 2 gruppo/sconosciuti · 3 web/tool-esterni. Il **taint del contesto** = max tier dei blocchi **fisicamente presenti nel context del turno** (messaggio + recall + tool output già ricevuti). Il taint **si propaga**: messaggio→fatto (il fatto eredita il tier), fatto→recall (l'etichetta viaggia), recall→azione (il kernel legge il taint prima di eseguire). Risposta alla domanda diretta del BRIEF: sì, serve il taint, ed è un campo persistito, non una convenzione di prompt. **Il filesystem locale non ha provenienza** (ADR-0044): `stat` non distingue `~/appunti.md` scritto dall'owner da `~/Downloads/fattura.pdf` arrivato da uno sconosciuto, quindi ciò che entra dal disco — `fs_read`, `fs_list`, lo stdout di `shell_run` — è **tier 2**; la tabella dei processi (`process_list`, solo `comm`) è tier 1, perché sceglierne il contenuto costa a un attaccante l'esecuzione di codice sull'host. Un risultato di tool fatto solo di parole del tool stesso è tier 0, **dichiarato** e non omesso: il campo è obbligatorio, così che «pulito» e «nessuno ha chiesto» non siano più lo stesso valore.
...
- **Ereditarietà del tier: invariante trasversale, non regola di una pipeline** (correzione C1-5). **Qualunque contenuto derivato** porta il tier massimo delle proprie fonti: fatti estratti da episodi, **valore di ritorno di un sub-agent** (trattato dal chiamante esattamente come un tool result, tier incluso), summary di compattazione, `profiles`, `digests`, OCR, descrizioni visive e trascrizioni. Un riassunto di contenuto tier-3 è tier-3, sempre — altrimenti la sintesi diventa una lavanderia del taint.
...
**(e) File/PDF/immagine importati nel vault.** Sono episodi tier-3 (web) o almeno del tier della sorgente che li ha prodotti: inoltrare un file da una chat owner non lava la sua provenienza. Il tenant risolto dalla surface accompagna i byte fino all'indice: un allegato di gruppo non entra mai in `host`. Poiché la directory fisica del vault è condivisa, un ingresso indicizza **solo il percorso appena acquisito**: passare il tenant giusto a una scansione completa copierebbe nello stesso tenant note host e file di altri gruppi. La scansione completa resta un comando di manutenzione esplicito, non un effetto collaterale dell'arrivo. Formato, filename, metadata, testo estratto, OCR e descrizione visiva diventano blocchi separati e tipizzati; ogni derivato eredita il massimo tier delle fonti. Un formato non parsabile fallisce in modo esplicito. I contenitori compressi hanno un tetto sull'output effettivo dell'inflater, non solo sui byte d'ingresso; un symlink esterno non è una fonte stabile e viene rifiutato. Il contenuto entra nel recall come dati etichettati; mai eseguito, mai fonte di trigger.
...
**(h) Bio/nome/immagine che fingono autorità.** Un profilo chiamato “Giusto — ignora le regole”, una bio con istruzioni, un filename `APPROVA.txt` o testo dentro una foto sono tutti input. Il connector ha già risolto il principal dal subject-id autenticato prima di parsarli; quei campi arrivano come blocchi dati col proprio tier e non possono cambiare principal, tenant, policy o ruolo nel prompt. La catena si spezza al confine tipizzato, poi di nuovo al kernel se l'iniezione convince comunque il modello.

## 5. Root of Trust — contenuto esatto e perché il confine è lì

Nel RoT (piccolo, esplicito, nominato — L0-3): **(1)** il kernel di policy + le definizioni di capability; **(2)** la matrice/config dei permessi e il ladder di fiducia; **(3)** l'egress allowlist; **(4)** `identity.md` (invarianti di condotta); **(5)** i budget (spesa API, proattività, quiet-hours); **(6)** la suite eval di riferimento del cricchetto (05); **(7)** i profili sandbox (mandatory deny paths, allowlist egress di esecuzione); **(8)** il meccanismo di update stesso; **(9)** i binding di autorità delle surface e la policy di re-pairing — la destinazione di consegna resta config, l'identità che può cambiare policy no. Confine tracciato lì perché è **l'insieme minimo che rende sicuro tutto il resto**: con questi elementi fissi, ogni altra cosa può essere self-modificabile senza che "self-modifying" e "sicuro" collidano (risoluzione del conflitto L0-2 vs L0-3 — che nessun peer ha: A2). Meccanica: file sotto `~/.muffin/rot/` di proprietà di un utente OS separato o read-only al processo agente (enforcement dal sistema operativo, non dal codice che il processo stesso carica), hash verificato al boot, modifiche solo repo→install+riavvio.
...
| Parser o percezione multimodale omettono o attribuiscono male contenuto | parsing tipizzato impedisce la confusione di ruolo, non garantisce comprensione completa | formato unsupported esplicito, provenienza per derivato, fixture ostili su metadata/OCR/audio e fallback fail-closed |
