...
| C2 | Extraction | L'estrazione è automatica? | BLOCKER — solo scenario mancante: consolidamento cablato a fine turno (`agent/runtime.ts:619`, `core/memory/ingest.ts:182`), debounce 20s misurato sul corpus reale; nessuno scenario di accettazione → PERCORSO-CRITICO.md#da-qui-ordina-luso (J1) |
| C3 | Consolidation | Si consolida senza intervento? | BLOCKER — solo scenario mancante: drain a pagina piena, dedup a chiave esatta, `muffin memory review` (`core/memory/consolidator.ts`), misurato per STATE.md ma non provato in `evals/acceptance/`; nessuno scenario → PERCORSO-CRITICO.md#da-qui-ordina-luso (J1) |
...
| C5 | Provenance | Posso capire **perché** crede una cosa? | BLOCKER — `muffin memory why` esiste per l'owner (`cli/memory.ts:29`, `core/memory/store.ts:918 provenanceOf`), ma non è esposto come tool-agente (`agent/tools/memory.ts` ha solo `memorySearchSpec`); nessuno scenario → PERCORSO-CRITICO.md#da-qui-ordina-luso (J2) |
| C6 | Temporal graph | «Chi era X a maggio» | BLOCKER — solo scenario mancante: `factsAsOf`/`nearestFactTo` (`core/memory/store.ts:545,581`) e `asOf` come parametro unico (`core/memory/recall.ts:167-236`) cablati sia in CLI sia nel tool; nessuno scenario di accettazione → PERCORSO-CRITICO.md#da-qui-ordina-luso (J1) |
| C7 | PDF | Acquisisce documenti utili? | BLOCKER — solo scenario mancante: PDF/DOCX/testo interi (`core/documents/extract.ts`), percorso allegato→vault→reindex→episodio provato da `connectors/telegram/document-arrival.test.ts` (non-acceptance, 326 righe); fallimento esplicito su scansioni senza testo; manca lo scenario in `evals/acceptance/` → PERCORSO-CRITICO.md#da-qui-ordina-luso (J2) |
...
> **rosso** prima del fix (PRACTICES.md#model-judgement-and-deterministic-contracts-stay-separate). Dettaglio in `docs/lessons.md`
...
| D4 | Shell | Esegue comandi nel sandbox? | BLOCKER — solo scenario mancante: "? su Linux" è stale, chiuso il 15/08 (PR #13 `slice/sandbox-linux`); CI Linux verde su `dev@3068ece` (run 32016357127, job `verifica`, 2026-08-17T09:39:47Z); la prova vive in CI, va portata nell'harness di accettazione o dichiarata equivalente → PERCORSO-CRITICO.md#da-qui-ordina-luso (J6) |
| D5 | Process | Gestisce processi lunghi? | BLOCKER — solo scenario mancante per list/kill: `process_list`/`process_kill` tipizzati (`agent/tools/process.ts:47-97`), `sys.process.kill` è ASK in single-user; nessuno scenario → PERCORSO-CRITICO.md#da-qui-ordina-luso (J6). Avviare processi propri in background resta OUT (nessuna capability dei 14 giorni lo richiede) |
...
| D11 | Checkpoint | Esiste uno snapshot prima di ogni mutazione, e un ripristino che disfa anche il turno? | BLOCKER — **metà chiusa da `slice/undo-journal`, metà no, e la riga chiede entrambe.** Lo snapshot pre-effect esiste (D2): nessuna capability `undoable` esegue senza che la copia sia già sul disco, e il WAL dell'intento era già chiuso da PR [#57](https://github.com/GiustoPiedimonte/muffin-agent/pull/57) (`startToolCall` che fallisce impedisce l'handler, mutazione verificata in `agent/turn-record.test.ts`). **Resta il turno.** `muffin undo` riallinea il filesystem e non tocca il record del turno né la sessione: dopo un undo la cronologia dice ancora «ho scritto nuovo.txt» e il modello ci crede, quindi la sua idea del mondo e il mondo divergono in silenzio — che è precisamente ciò che l'owner intendeva il 16/08 chiedendo un undo che riallinei «il filesystem **e** il turno» (§1). Non è decorazione: il consumatore è il giro dopo. → prossima claim, `slice/undo-riallinea-il-turno` (CRITICAL) |
...
| E7 | Self-inspection | Sa spiegare **tecnicamente** come funziona e cosa sta usando **adesso**, distinguendo architettura/progetto da stato live dell'istanza? | BLOCKER — **meccanismo atterrato, acceptance bloccata su una decisione** (#176). `sys.inspect` esiste, è first-class, read-only, `hostOnly`, e legge dalle stesse fonti autorevoli: `runDoctor()` per i check, `describeBuild()` per il build, `TurnStore.health()` per i turni, `JobStore.list()` per i job, la config e i `promptBlocks` del runtime — nessuna rilettura propria. Non inoltra i `detail` dei check falliti, così `tier: 0` resta vero (il messaggio di una sonda è testo di terze parti). **Ciò che manca non è codice**: su `consumer-local` (`maxToolsExposed: 10`, la soglia contro cui è disegnato l'harness) i tool registrati sono dodici, quindi `sys_inspect` è oltre il tetto e invisibile al modello — cioè invisibile sull'installazione dell'owner, che gira `qwen/qwen3.8-27b`. Il taglio è **dichiarato** in `bootLines` invece che silenzioso (ADR-0008). Farlo entrare costerebbe uno dei dieci — e il candidato naturale, `http_get`, è escluso per disegno da `agent/runtime-exposure.test.ts`. È una decisione prodotto con un prezzo, non un ordine di `push`: **domanda aperta all'owner**. Su un profilo frontier (tetto 24) il tool è esposto e l'acceptance è eseguibile. Lacuna originale (owner, 17/08): il modello può solo recitare ciò che il prompt dice o indovinare; nessuna primitiva read-only lo lascia interrogare runtime, provider/modelli correnti, surface/tenant, RoT/safe mode, sandbox/search/MCP disponibili, capability esposte, blocchi del prompt e provenienza, modalità reale della memoria (indice vettoriale disponibile o degradato), turni aperti/waiting/interrupted, job essenziali. Forma decisa: **`sys.inspect`** first-class e read-only che legge dalle **stesse fonti autorevoli** di `doctor` / `prompt show` / `gateway status` (una sola source of truth, nessuna implementazione divergente, niente documentazione infilata nel system prompt); acceptance: «spiegami tecnicamente come funzioni e cosa stai usando adesso» → cambia una condizione reale (search off, modello diverso, MCP assente) → ripeti: se recita lo stato vecchio è BROKEN, se distingue design e live state è verde → PC 3 |
...
> è ancora `atteso-rosso` (MANDATO-DAY-1.md#day-1-ready — le due affermazioni non possono
...
## La direzione oltre il Gate 1 non allarga il Gate 1
