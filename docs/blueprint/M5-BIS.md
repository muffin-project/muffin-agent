...
| D11 | Checkpoint | Esiste uno snapshot prima di ogni mutazione, e un ripristino che disfa anche il turno? | BLOCKER — **metà chiusa da `slice/undo-journal`, metà no, e la riga chiede entrambe.** Lo snapshot pre-effect esiste (D2): nessuna capability `undoable` esegue senza che la copia sia già sul disco, e il WAL dell'intento era già chiuso da PR [#57](https://github.com/GiustoPiedimonte/muffin-agent/pull/57) (`startToolCall` che fallisce impedisce l'handler, mutazione verificata in `agent/turn-record.test.ts`). **Resta il turno.** `muffin undo` riallinea il filesystem e non tocca il record del turno né la sessione: dopo un undo la cronologia dice ancora «ho scritto nuovo.txt» e il modello ci crede, quindi la sua idea del mondo e il mondo divergono in silenzio — che è precisamente ciò che l'owner intendeva il 16/08 chiedendo un undo che riallinei «il filesystem **e** il turno» (§1). Non è decorazione: il consumatore è il giro dopo. **Chiusa la seconda metà da `slice/undo-riallinea-il-turno`** (CRITICAL): `turn_tool_calls` ha una colonna di ritiro `undone_at` — stessa forma di `episodes.superseded_at` e `facts.expired_at`/`superseded_by`, la riga non si riscrive e smette di essere presentata come corrente — `cli/undo.ts` la segna dopo il restore e solo per i `callId` davvero rimessi indietro, e i due lettori la presentano annullata: `buildContext` per il giro dopo (via `TurnStore.undoneTurns` sui `traceId`, lo stesso join che `history-taint.ts` usa per il tier) e la ripresa per il turno stesso (trascrizione durevole e replay di `recordedOutcomes`). Provato end-to-end attraverso `buildRuntime` in `agent/runtime-wiring.test.ts`: dopo un `muffin undo` il contesto del turno successivo non contiene più l'affermazione della scrittura senza la sua smentita. **Resta fuori un canale**: la risposta dell'agente è anche un episodio di memoria (`agent/loop.ts` `addEpisode`, che nel suo stesso commento dice «tomorrow's recall fishes this sentence back out»), e non esiste oggi un join turno→episodio su cui appoggiare lo stesso mark — follow-up dichiarato, non coperto qui. |
