...
| B13 | Progress | Un turno lungo dice di essere vivo in modo **strutturale**, non cosmetico? | BLOCKER — il meccanismo è in HEAD (#136, judge MERGE): `TurnInput.onProgress` accanto a `onDelta`, con `TurnEvent` (`round`/`model`/`tool_start`/`tool_end`) emesso dagli stessi punti che scrivono gli span, quindi canale e trace non possono divergere. Separato da `onDelta` di proposito: quello porta solo la risposta finale ed è bufferizzato fino al round terminale per non ritrattare testo, mentre il progresso riguarda esattamente i round intermedi — e un evento di progresso racconta ciò che è **già** successo, quindi non si ritratta mai. Resta BLOCKER per la copertura delle superfici: solo il REPL lo cabla (su stderr, con gate `isTTY`); **Telegram non lo riceve** — non filtrato, proprio non cablato — e là «una riga per evento» è la forma sbagliata (rate limit Bot API: serve il draft+edit già usato per `onDelta`). `turns.updated_at` resta il segnale durevole per il *resume*, complementare e non sostitutivo |
