...
- **`slice/segreti-mai-mostrabili`** (**CRITICAL**, direttiva owner 17/08) — un
  secret value conosciuto non entra mai nel data plane generale: si materializza
  solo nel sink privilegiato che ne ha bisogno, il più tardi possibile, e da lì
  non torna nel runtime osservabile. `secret://…` è ciò che viaggia. Chiude la
  decisione owner su **P34-2**: redazione al confine di scrittura come
  defense-in-depth, non come garanzia. In corso.
...
`init --local` · #60 citazioni della mappa · #63 workflow (profili di verifica) · #61 cluster di MEDIUM dell'audit (P34-1, P35, P36, P25, P33, E2) · #62 egress-params (inv. 7, CRITICAL: judge MERGE, soglia owner = 2) · #65 character eval (A2/A3 parte 2) · #67 session-taint (inv. 2, CRITICAL: judge MERGE).
