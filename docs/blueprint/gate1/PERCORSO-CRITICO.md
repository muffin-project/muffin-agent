...
- **`slice/job-fires`** (PC 1.5, **CRITICAL**) — l'ultima del blocco 1: ogni
  occorrenza dovuta `(job_id, scheduled_for)` mappa a **una sola** identità
  durevole di turno; dopo un crash Muffin continua o conclude quella, non ne crea
  una seconda e non abbandona la prima (decisione owner 17/08, matrice di fault a
  sette punti). In corso.
