...
| D16 | Taint usabile | Dopo una ricerca web, nella **stessa** conversazione, shell e scrittura restano usabili, e il taint compare come ragione nelle domande invece che come muro? | BLOCKER — misurato il 06/09 sul `muffin.db` dell'owner: nove turni su quattordici in privato a taint 3, ultima shell vera il 03/09, ultimo turno finito con `context taint 3 exceeds 2 for sys.shell (host)`. Decisione owner: «inutilizzabile in sto modo». ADR-0075 (proposto): la riga `host` non nega più per taint (ogni sua capability è reversibile o chiede comunque), i `maxTaint` spariscono dalle capability reversibili, verso l'esterno il taint chiede all'owner e nega agli altri, e resta provenienza ovunque. **Criterio eseguibile:** (1) `core/policy/solo-irreversibile.test.ts` enumera ogni dichiarazione spedita anche a taint 3 e nessuna della riga `host` risponde `taint_exceeded`; mutazione `host.denyAbove: 2` → rosso; (2) uno scenario di accettazione fa `web_search` e poi `shell_run` nello stesso turno, headless, sul binario vero, e la shell risponde; (3) il corpus di `evals/security/` rieseguito non alza il numero di attacchi che completano senza umano (4/8). Entra dopo #456 e #457, che toccano gli stessi file. |
