...
| A2 | Identity | Sa chi è e quali limiti ha? | BLOCKER 👤 il contenuto non è più il gap: `identity.md` è testo reale dell'owner da c090dce, sigillata nel RoT, e raggiunge il system prompt reale — provato (`slice/identita` parte 1: `docs/evidence/prompt-assembly-2026-08-17.md`, `muffin prompt show`, wiring test mutati rosso-prima, scenario di accettazione A2 verde contro il binario vero, `evals/acceptance/scenarios/a-lifecycle.accept.ts`). Resta BLOCKER: manca il character eval sui modelli DAY-1 (punto 6 mandato owner — non più «solo owner»: il contenuto è dato, la verifica no) → parte 2 di `slice/identita`. **03/09: il metro esiste ed è agganciato all'assemblaggio vero** — 22 probe (`evals/character/`), fra cui le cinque esche in cui il fallimento è plausibile (adulazione, sentimento finto, un numero inventato, un'azione mai compiuta raccontata al passato, un limite spiegato con una policy che non esiste), e un test che confronta **byte per byte** il prompt che l'eval manda con `renderSystemPrompts(buildSystemPromptBlocks(home)).owner` (rosso-prima verificato sostituendo `deps.systemPrompts` con una ricostruzione). Una corsa locale (gemma4:12b, giudice qwen3:8b) era stata fatta girare lo stesso giorno ma le sue trascrizioni erano scratch e non sono mai state committate: `docs/evidence/character-baseline-2026-09-03.md` non esiste nel repo, e questa riga lo citava come se ci fosse. Quello che è vero e committato oggi: il metro gira dentro `npx vitest run` (quindi nello step `verifica` di CI) e ha uno script proprio (`npm run eval:character`); `--dry-run` prova che i 22 probe, la rubrica e l'assemblaggio non sono rotti, e non prova niente sul carattere. Nessuna corsa reale, locale o a pagamento, è stata autorizzata o eseguita in questa slice — la riga resta BLOCKER e resta dell'owner → tracked in issue #359 |
| A3 | Persona | Il comportamento è definito? | BLOCKER 👤 `persona.md`/`voice.md` sono testo reale dell'owner da c090dce e raggiungono il system prompt reale nell'ordine canonico (persona → identity → voice) — provato: scenario A3 verde, `muffin prompt show` byte-identico a quanto ricevuto davvero dal provider (`evals/acceptance/scenarios/a-lifecycle.accept.ts`). Resta BLOCKER: mancano character eval, cross-model e confronto col vecchio `Muffin.ai` (punti 6-8 mandato owner) → parte 2 di `slice/identita`. **03/09: il metro è ora sulla strada, la baseline no.** `evals/character/` è a 22 probe, ha uno script proprio (`npm run eval:character`) e gira dentro `npx vitest run` — quindi nello step `verifica` di CI, in `--dry-run` e su fixture, mai in rete. Una corsa locale (gemma4:12b, giudice qwen3:8b) era stata fatta girare lo stesso giorno e mostrava il carattere non tenere proprio dove il fallimento è plausibile — la voce da assistente generico vietata per nome in `voice.md` sul saluto più banale — ma le sue trascrizioni erano scratch e non sono mai arrivate nel repo: non c'è un `docs/evidence/character-baseline-2026-09-03.md` da citare, e questo va detto invece di lasciarlo implicito. Un `--dry-run` non prova niente sul carattere, solo che i probe e l'assemblaggio non sono rotti; nessuna corsa reale — sui modelli DAY-1 o altrove — è stata autorizzata o eseguita in questa slice. La riga resta BLOCKER ed è dell'owner → tracked in issue #359 |
