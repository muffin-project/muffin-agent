...
> #186 chiusa; dipende dal soffitto di `fs.write`), E7 (il tetto è 15,
...
> felice senza provider veri), D11 (soffitto di `fs.write`), E1 (per-job), E5
...
| A6 | Upgrade | Aggiornare il codice non distrugge dati? | BLOCKER — solo scenario mancante: il meccanismo c'è (RETURN S2, judge CRITICAL MERGE giro 2): runner versionato `core/db/migrate.ts` con `schema_version`, guardia `SchemaAheadError` (codice vecchio su dati nuovi rifiuta), backup `VACUUM INTO` validato prima del primo reshaping, `stampFresh` per install fresche; **il verbo `muffin update` esiste** (`cli/update.ts`, da 9f56484 del 26/08: backup prima dello swap atomico del launcher — riconciliato il 02/09: la riga diceva ancora «resta il verbo»); manca lo scenario di accettazione della journey install/update/backup sul binario reale → DOGFOOD (critical-path.md#da-qui-ordina-luso) |
| A7 | Migration | Lo schema evolve senza perdere memoria? | BLOCKER — la trappola CHECK è chiusa nel meccanismo (RETURN S2): `rebuildTable` prova su DB popolato che il CHECK si allarga con righe intatte e constraint nuova attiva, e abortisce se perde righe; regole per il primo autore di migrazione nel docstring di `MIGRATIONS` (tabelle surface lazy, storia non-additiva di `vectors.ts`); resta lo scenario di accettazione → DOGFOOD (critical-path.md#da-qui-ordina-luso) |
| A8 | Backup | La memoria si salva e si ripristina? | BLOCKER — A8-min chiuso (RETURN S2): `muffin backup` online WAL-safe con `quick_check`, `muffin restore` che rifiuta gateway vivo e backup più nuovi del codice, aside `VACUUM INTO` (una riga rimasta solo nel `-wal` di un writer ucciso sopravvive — regression test SIGKILL in `cli/backup.test.ts`); provato sul binario reale. Riconciliato il 02/09: lo scenario `A8` del manifest esiste ma copia la home con `cpSync` (`a-lifecycle.accept.ts`), **non esercita i verbi** `muffin backup`/`muffin restore` — la prova dei verbi è unit, non accettazione; restano matrice hot-backup sotto carico, retention/cron e lo scenario sui verbi → DOGFOOD (critical-path.md#da-qui-ordina-luso) |
...
| B6 | Retry | Se fallisce una tool call, recupera? | BLOCKER — solo scenario mancante: il retry per-tool esiste (`eseguiConRitentativi` in `agent/loop.ts`, `MAX_TOOL_RETRIES=2`, da d257285 del 28/08: ritenta solo un outcome `retryable` su una capability `rerunnable`; `http.ts` e `search.ts` marcano transienti/5xx, `fs.ts` no — un errore su disco non è transiente), oltre al retry di trasporto/modello (`MAX_TRANSPORT_RETRIES=2`); provato da `agent/tool-retry.test.ts`; nessuno scenario di accettazione → critical-path.md#da-qui-ordina-luso (riconciliato il 02/09: la riga diceva «nessun tool implementa retry proprio») |
...
| B13 | Progress | Un turno lungo dice di essere vivo in modo **strutturale**, non cosmetico? | BLOCKER — **solo scenario mancante**. Il meccanismo è in HEAD (#136, judge MERGE): `TurnInput.onProgress` accanto a `onDelta`, con `TurnEvent` (`round`/`model`/`tool_start`/`tool_end`) emesso dagli stessi punti che scrivono gli span, quindi canale e trace non possono divergere. Separato da `onDelta` di proposito: quello porta solo la risposta finale ed è bufferizzato fino al round terminale per non ritrattare testo, mentre il progresso riguarda esattamente i round intermedi — e un evento di progresso racconta ciò che è **già** successo, quindi non si ritratta mai. **Entrambe le superfici lo cablano ora**: REPL su stderr con gate `isTTY` (`cli/repl.ts:439`), Telegram con draft+edit e non una riga per evento (`connectors/telegram/progress.ts`, cablato in `connector.ts:660-708`) — un solo messaggio, throttled a `MIN_EDIT_MS`, mai una cronologia, e ripulito a fine turno. Tre difetti trovati dal judge fresco su quel cablaggio e chiusi con mutazione (#143, #145): invii concorrenti che riordinavano le righe; la serializzazione che *introduceva* un'attesa senza tetto su `stop()`, pagata dall'owner **prima** della risposta vera (ora `STOP_WAIT_MS` = 2s: oltre, meglio una riga cosmetica di troppo che trattenere la risposta); la guardia ABA della catena non provata da nessuno dei 17 test (serve uno scenario a **tre** anelli, non due). `turns.updated_at` resta il segnale durevole per il *resume*, complementare e non sostitutivo. Resta BLOCKER perché nessuno scenario di accettazione lo esercita sul binario vero: `e2e-giro-owner.accept.ts:378` si limita a tollerare il `sendMessage` in più |
| B14 | Attachment | Un file prodotto arriva come **allegato**, o come percorso da copiare a mano? | BLOCKER — solo scenario mancante: `send_file` (`agent/tools/deliver.ts:58`) raggiunge `Surface.deliverFile` (`core/surface/registry.ts:58-61`) su Telegram e Discord, `hostOnly` dichiarato (⚠️ un member non può ricevere un proprio file, vedi sotto); manca lo scenario di accettazione → critical-path.md#da-qui-ordina-luso |
...
| C2 | Extraction | L'estrazione è automatica? | BLOCKER — solo scenario mancante: consolidamento cablato a fine turno (`agent/runtime.ts:619`, `core/memory/ingest.ts:182`), debounce 20s misurato sul corpus reale; nessuno scenario di accettazione → critical-path.md#da-qui-ordina-luso (J1) |
| C3 | Consolidation | Si consolida senza intervento? | BLOCKER — solo scenario mancante: drain a pagina piena, dedup a chiave esatta, `muffin memory review` (`core/memory/consolidator.ts`), misurato per STATE.md ma non provato in `evals/acceptance/`; nessuno scenario → critical-path.md#da-qui-ordina-luso (J1) |
...
| C6 | Temporal graph | «Chi era X a maggio» | BLOCKER — solo scenario mancante: `factsAsOf`/`nearestFactTo` (`core/memory/store.ts:545,581`) e `asOf` come parametro unico (`core/memory/recall.ts:167-236`) cablati sia in CLI sia nel tool; nessuno scenario di accettazione → critical-path.md#da-qui-ordina-luso (J1) |
| C7 | PDF | Acquisisce documenti utili? | BLOCKER — solo scenario mancante: PDF/DOCX/testo interi (`core/documents/extract.ts`), percorso allegato→vault→reindex→episodio provato da `connectors/telegram/document-arrival.test.ts` (non-acceptance, 326 righe); fallimento esplicito su scansioni senza testo; manca lo scenario in `evals/acceptance/` → critical-path.md#da-qui-ordina-luso (J2) |
| C8 | Audio | Gestisce le note vocali DAY-1 conservando audio originale e provenance del transcript? | BLOCKER — **REAL-ENV, meccanismo atterrato**: `core/audio/voce.ts` decide per modello (`audioAccettato` interroga `{baseUrl}/models`) e `core/audio/trascrivi.ts` trascrive in casa con whisper.cpp + ffmpeg, cablato nel path vocale Telegram (`connector.ts`, transcript recintato come dato tainted; `voice-arrival.test.ts`) — da 51192bd del 28/08; la riga diceva «non esiste transcriber». Misurato il 02/09 sull'installazione dell'owner: `qwen/qwen3.8-27b` dichiara `["text","image","video"]` (niente audio), `whisper-cli` e `ffmpeg` assenti, nessun `~/.muffin/models/ggml-base.bin`, e `muffin doctor` non ha una riga che lo dica: la prima nota vocale fallirebbe con il rimedio stampato a posteriori. Manca: check `doctor` proattivo, prerequisiti installati sulla macchina reale, scenario → PC 3 |
...
| D4 | Shell | Esegue comandi nel sandbox? | BLOCKER — solo scenario mancante: "? su Linux" è stale, chiuso il 15/08 (PR #13 `slice/sandbox-linux`); CI Linux verde su `dev@3068ece` (run 32016357127, job `verifica`, 2026-08-17T09:39:47Z); la prova vive in CI, va portata nell'harness di accettazione o dichiarata equivalente → critical-path.md#da-qui-ordina-luso (J6) |
| D5 | Process | Gestisce processi lunghi? | BLOCKER — solo scenario mancante per list/kill: `process_list`/`process_kill` tipizzati (`agent/tools/process.ts:47-97`), `sys.process.kill` è ASK in single-user; nessuno scenario → critical-path.md#da-qui-ordina-luso (J6). Avviare processi propri in background resta OUT (nessuna capability dei 14 giorni lo richiede) |
...
| D11 | Checkpoint | Esiste uno snapshot prima di ogni mutazione, e un ripristino che disfa anche il turno? | BLOCKER — metà lineage rientrata (#253: `episodes.turn_id`, `RecallItem.turnId`, esclusione del recall per lineage); metà «l'undo riallinea il turno» **non rientrata**: PR #186 chiusa senza merge il 30/08 come SALVAGE (141 commit indietro, 5 conflitti), il ramo `slice/undo-riallinea-il-turno` resta come miniera, non si rebasa. La misura che l'ha chiusa vale ancora il 02/09: sul `muffin.db` dell'owner `fs.write` **non è mai stata eseguita** (0 chiamate su 165 turni), perché 110 turni su 165 sono a taint 2 e `fs.write` ha soffitto 1 — il percorso che questa metà ripara non è raggiunto finché la decisione owner sul soffitto (critical-path.md#decidere-il-workflow-locale-read--write) non lo apre. Dipende da quella; poi si reimplementa su HEAD |
| D12 | Ask | L'ASK mostra **cosa** sta per fare (comando+cwd, URL, pid+nome) e perché il turno è a quel taint? | BLOCKER — il meccanismo c'è (RETURN S3): per le capability `resourceKind: 'none'` la richiesta deriva la risorsa dagli argomenti della call (`summarizeCallArgs`, `agent/loop.ts`), `ApprovalRequest` porta il taint del turno, il REPL e il rendering in coda dello scheduler mostrano azione e taint; la coda durevole degli ask esiste e ha un consumer (`core/approvals/store.ts`): l'ASK su una superficie asincrona scrive una riga, sospende il turno sulla barriera `approval:<id>` e riprende quando la risposta arriva — anche dopo un riavvio; su Telegram la risposta sono due pulsanti (`cli/surface.ts`, `connectors/telegram/connector.ts`), e l'approvatore è instradato per superficie, non uno per processo. Una risposta vale **una volta**, su quella capability e quella risorsa. Resta senza scenario di accettazione end-to-end → DOGFOOD |
...
| E7 | Self-inspection | Sa spiegare **tecnicamente** come funziona e cosa sta usando **adesso**, distinguendo architettura/progetto da stato live dell'istanza? | BLOCKER — solo scenario mancante: il meccanismo è atterrato (#176) e **la decisione che bloccava l'acceptance è decaduta**. `sys.inspect` è first-class, read-only, `hostOnly`, e legge dalle stesse fonti autorevoli (`runDoctor()`, `describeBuild()`, `TurnStore.health()`, `JobStore.list()`, config e `promptBlocks`), senza inoltrare i `detail` dei check falliti (`tier: 0` resta vero). Riconciliato il 02/09: `consumer-local` ha `maxToolsExposed: 15` da 0d519cb (28/08), `sys_inspect` è esposto sull'installazione dell'owner — 21 chiamate reali nel `muffin.db`, `doctor` non stampa più «invisibili al modello» (la riga citava tetto 10 e 12 tool). Lacuna originale (owner, 17/08): nessuna primitiva read-only lasciava al modello interrogare runtime, provider, surface, RoT, sandbox, capability, prompt e memoria vivi. Acceptance: «spiegami tecnicamente come funzioni e cosa stai usando adesso» → cambia una condizione reale (search off, modello diverso, MCP assente) → ripeti: se recita lo stato vecchio è BROKEN, se distingue design e live state è verde → PC 3 |
