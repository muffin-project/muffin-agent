...
> **Riconciliazione owner 08/09/2026.** Foundation e cutover restano provati;
> il gate corrente ha tre `BLOCKER`: scheduling owner-facing preciso (B20),
> Work-liveness sicura (B21) e privacy request-egress (D17). I conteggi datati
> sopra sono cronaca; questa tabella è lo stato corrente e
> `critical-path.md` ne possiede soltanto l'ordine.
...
| B20 | Scheduling owner-facing | Da qualunque surface normale, «domani alle 08:32 ricordami X» e un reminder ricorrente deterministico arrivano sulla surface di default al momento richiesto, senza LLM al fire? | BLOCKER — il one-shot `todo.due` → `CommitmentLane` è durevole e no-LLM, ma viene osservato sul beat di 30 s; la creazione dei job ricorrenti è ancora solo CLI/cron. `Job.kind='script'` esiste già: sandboxed, no-LLM, stdout vuoto = silenzio; va preservato e dogfooded, non ricostruito. Maintenance heartbeat e precise timer sono primitive diverse. |
| B21 | Work-liveness | Senza un nuovo messaggio owner, un wake di maintenance avanza Work durevole actionable attraverso il runtime normale, restando silenzioso e senza LLM quando non c'è nulla da fare? | BLOCKER — Work, resume e scheduler esistono; manca la claim integrata `WAKE → OBSERVE → CHOOSE → WORK → VERIFY → CONTINUE`. Non autorizza un secondo runtime, `GoalEngine` o `HeartbeatManager`. |
...
| D17 | Privacy request-egress | Il catalogo modelli resta separato dall'idoneità privacy della singola richiesta, valutata localmente sull'intera richiesta assemblata prima del route? | BLOCKER — oggi il provider configurato riceve l'intero contesto. Target: secret locali/fail-closed; PII transform non equivale a privacy semantica; significato sensibile irrisolto prende route/policy più forte. Il minimo meccanismo è una futura claim `DELEGATE`, non questo reset. |
