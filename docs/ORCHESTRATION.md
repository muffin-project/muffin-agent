...
Integration into `dev` goes through green GitHub checks on the PR head
(`docs/BRANCHING.md` §3): merge only when every check is green and none is
pending. Fino al 17/09 valeva la porta locale (`npm run merge -- <pr>`,
risultato unito in un worktree usa-e-getta, `ci:local`, merge solo su PASS)
con hook a farla rispettare — rimossa su decisione owner insieme alla porta.
Resta `npm run ci:local` per rieseguire gli stessi job in Docker a mano. A
direct `gh pr merge` su check rossi o pending non è più bloccato da niente
fino al source-public: la garanzia è disciplina, e va detta.
