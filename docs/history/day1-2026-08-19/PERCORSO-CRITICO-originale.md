...
*(Verificata meccanicamente: `node .claude/riconcilia.mjs` esce ≠ 0 se qui
compare una PR già mergiata o un branch che non esiste più. Va aggiornata **nello
stesso passaggio del merge** — `BRANCHING.md` checkpoint 4. Il controllo non vede
una voce *duplicata*: quella la vede solo chi legge.)*

**Il blocco 1 è chiuso.** Le sei invarianti trasversali che potevano invalidare
più righe insieme sono integrate: WAL dell'intento (#57), taint attraverso la
history (#67), egress che ispeziona i byte (#62), segreti fuori dal data plane
(#70), provenienza in ingresso (#72), identità dell'occorrenza (#76).

- **PR #73** `slice/product-open-source-direction` (FAST, altra sessione) —
  direzione prodotto/open-source, solo documenti. Nota aperta: crea `research/`
  alla radice mentre la convenzione (`PRACTICES.md` §13) è
  `docs/blueprint/research/`.

**Prossimo**: `inbound-unit` (la seconda metà di 1.5 — `update_id` → un solo
turno durevole su Telegram, sulla forma `claim`/`bind`/`settle` che
`job_fires` ha lasciato pronta), poi il **blocco 2**: forma durevole prima che arrivino i dati
veri (schema-evolution, update/backup, undo-journal).
...
`init --local` · #60 citazioni della mappa · #63 workflow (profili di verifica) · #61 cluster di MEDIUM dell'audit (P34-1, P35, P36, P25, P33, E2) · #62 egress-params (inv. 7, CRITICAL: judge MERGE, soglia owner = 2) · #65 character eval (A2/A3 parte 2) · #67 session-taint (inv. 2) · #70 segreti mai mostrabili (argv, env, MCP, stdin) · #72 ingress-forward (inv. 3 in ingresso, B16 minimo) · #71 deleghe riprendibili (harness, altra sessione) · #76 job-fires (inv. 4/5, CRITICAL: judge MERGE).
