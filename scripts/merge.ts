...
function main(): void {
  ...
  const info = JSON.parse(
    gh(['pr', 'view', pr, '--json', 'headRefName,baseRefName,state,headRefOid,title']),
  ) as { headRefName: string; baseRefName: string; state: string; headRefOid: string; title: string };
  if (info.state !== 'OPEN') {
  ...
  if (info.baseRefName !== 'dev') {
    ...
    process.stderr.write(`PR #${pr} ha base ${info.baseRefName}: questa porta unisce solo in dev\n`);
    process.exit(1);
  }
  ...
  try {
    const merge = spawnSync('git', ['-C', dir, 'merge', '--no-edit', headSha], { encoding: 'utf8' });
    if (merge.status !== 0) {
      esito = 'conflict';
      process.stderr.write(`CONFLITTO fra dev e la PR — risolvilo nel ramo, non qui:\n${merge.stdout}${merge.stderr}`);
      return;
    }
    const merged = git(['rev-parse', 'HEAD'], dir);
    process.stdout.write(`  unito ${merged.slice(0, 10)} — npm ci nel worktree…\n`);
    const ci = spawnSync('npm', ['ci', '--no-audit', '--no-fund'], { cwd: dir, stdio: ['ignore', 'ignore', 'inherit'] });
    if (ci.status !== 0) {
    ...
    writeFileSync(
      ...
      JSON.stringify({ pr, headSha, devSha, mergedSha: merged, esito, at: new Date().toISOString() }, null, 2),
    );
