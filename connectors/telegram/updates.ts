...
export class UpdateInbox {
   ...
   *
   * The operation stays semantically distinct from `scrubSettledPayload`
   * (atomic gate-refusal settlement is not generic post-settle scrubbing),
   * but both retire bodies through the one canonical `scrubStub`
   * representation — integrated against #584, no second constructor.
   */
  sealIgnored(updateId: number, at: string): void {
    this.db
      ...
      .run(at, at, scrubStub(updateId), updateId);
  }
