 ...
 *
 * ADR-0062 split that one branch into two: `url-read` (reading — `sys.http`,
...
// A group member: taint 2. Before ADR-0062 the egress branch refused this
...
describe('reading is open, through a real turn (ADR-0062)', () => {
  it.each([true, false])(
  ...
  it('a junk path argument cannot shadow the url — resourceFor reads policyArgs, not argument order', async () => {
    ...
    // was already inert before ADR-0062, and reading being open now means the
...
/**
 * `url` (acting) is untouched by ADR-0062: no shipped capability declares it
...
describe('acting is still gated, through a real turn — ADR-0062 opened reading, not the allowlist', () => {
