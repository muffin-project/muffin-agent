 ...
 *
 * Other outcomes (`ask`, `error`, `cap`, `budget`) do not persist enough of
 * their original wording to reconstruct it honestly, so callers receive an
 * explicit recovery failure instead of invented text.
 */
export function recoveredText(sessions: SessionStore, record: TurnRecord): string {
  if (record.outcome === 'answered') {
    ...
    for (let i = messages.length - 1; i >= 0; i--) {
      ...
      if (message.role === 'assistant' && message.traceId === record.id && message.content.trim() !== '') {
